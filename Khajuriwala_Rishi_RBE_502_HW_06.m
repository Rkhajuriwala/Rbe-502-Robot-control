...
% Tau = M_bar(q)*a+ C_bar(q,dq)*v + N_bar(q) - Kv*r
% On equating the above eqs
% M(q)ddq + C(q,dq)dq +N(q) = M_bar(q)*a+ C_bar(q,dq)*v + N_bar(q) - Kv*r
% ddq = a + dr
% dq = v + r
%
% M(q)(a + dr) + C(q,dq)(v + r) +N(q) = M_bar(q)*a+ C_bar(q,dq)*v + N_bar(q) - Kv*r
% M(q)(dr) + C(q,dq)(r) + Kv*r = (M_bar- M)*a+ (C_bar- C)*v + (N_bar- N)(q)
% M(q)(dr) + C(q,dq)(r) + Kv*r = Y(a,v,q)*q_tilda
